...
(function () {
  ...
  const DATA_URL = '/haze-demo/assets/data/projekty.json';
  ...
  const PRODUCT_LABELS = {
    'stajova-vrata': 'Livestock building gates',
    'vrata-prumysl': 'Gates for industry',
    'bvs-ventilace': 'BVS ventilace',
    'stresni-vetrani': 'Ridge ventilation',
    'plachtova-vrata': 'Curtain gates',
  ...
  function init() {
    ...
    if (typeof L === 'undefined') {
      mapEl.replaceChildren(el('p', { className: 'haze-mapa-error', text: 'The map could not be loaded (Leaflet missing).' }));
      return;
    }
    ...
    fetch(DATA_URL)
      ...
      .catch((err) => {
        ...
        mapEl.replaceChildren(el('p', { className: 'haze-mapa-error', text: 'The project data could not be loaded.' }));
      });
  }
  ...
  function buildPopupContent(p) {
    ...
    if (p.result_metric) {
      ...
      wrap.appendChild(el('strong', { text: 'Result: ' }));
  ...
  function renderList() {
    ...
    counter.appendChild(document.createTextNode(' completed projects'));
