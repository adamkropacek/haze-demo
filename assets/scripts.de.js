...
const FORM_ENDPOINT = '/haze-demo/form.php';
...
function showFormSuccess(form) {
  ...
  h3.textContent = 'Vielen Dank. Wir melden uns innerhalb von 24 Stunden.';
  const p = document.createElement('p');
  p.appendChild(document.createTextNode('In der Zwischenzeit können Sie uns anrufen unter '));
  ...
  p.appendChild(tel);
  p.appendChild(document.createTextNode(' (Mo-Fr 7:00-17:00).'));
  wrap.appendChild(h3);
  wrap.appendChild(p);
...
function mailtoFallback(f) {
  ...
  const body = [
    ...
    `Gesendet von: ${location.href}`,
    `Datum: ${new Date().toISOString()}`
  ].join('\n');
  const subject = 'Anfrage HAZE';
...
(function(){
  ...
  lb.innerHTML='<button class="plb-close" aria-label="Schließen">×</button><button class="plb-nav plb-prev" aria-label="Zurück">‹</button><img alt=""><button class="plb-nav plb-next" aria-label="Weiter">›</button><div class="plb-count"></div>';
