...
(function () {
  ...
  const PRODUCT_LABELS = {
    'stajova-vrata': 'Stalltore',
    'vrata-prumysl': 'Tore für die Industrie',
    'bvs-ventilace': 'BVS ventilace',
    'stresni-vetrani': 'Firstlüftung',
    'plachtova-vrata': 'Planentore',
  ...
  function init() {
    ...
    if (typeof L === 'undefined') {
      mapEl.replaceChildren(el('p', { className: 'haze-mapa-error', text: 'Die Karte konnte nicht geladen werden (Leaflet missing).' }));
      return;
    }
    ...
    fetch(DATA_URL)
      ...
      .catch((err) => {
        ...
        mapEl.replaceChildren(el('p', { className: 'haze-mapa-error', text: 'Die Projektdaten konnten nicht geladen werden.' }));
      });
  }
  ...
  function buildPopupContent(p) {
    ...
    if (p.result_metric) {
      ...
      wrap.appendChild(el('strong', { text: 'Ergebnis: ' }));
  ...
  function renderList() {
    ...
    counter.appendChild(document.createTextNode(' Projekte'));
